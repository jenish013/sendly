...
const getFileNameSummary = (files) => {
  ...
  if (list.length === 1) return list[0].originalName;
  if (list.length <= 3) return list.map(file => file.originalName).join(', ');
  return `${list.slice(0, 2).map(file => file.originalName).join(', ')} and ${list.length - 2} more files`;
};

const sendViaEmailJs = async (transfer, publicOrigin, config = getEmailConfig(), options = {}) => {
  if (!config.emailjsServiceId || !config.emailjsTemplateId || !config.emailjsPublicKey || !config.emailjsPrivateKey) {
    return getEmailRecipients(transfer, options).map(recipient => ({
      email: recipient.email,
      success: false,
      skipped: true,
      reason: 'EmailJS is not fully configured (service ID, template ID, public key, and private key are all required)'
    }));
  }
  ...
  const fileNameSummary = getFileNameSummary(transfer.files);
  const fileSizeSummary = formatFileSize(transfer.totalSize);
  ...
  for (const recipient of recipients) {
    try {
      const response = await emailjs.send(
        ...
        {
          receiver_name: recipient.name || recipient.email,
          sender_name: transfer.senderName,
          file_name: fileNameSummary,
          file_size: fileSizeSummary,
          expires_at: expiresDate,
          download_link: transferUrl,
      ...
      const errorMessage = error?.text || error?.message || 'EmailJS send failed';
      logger.error(`EmailJS error for ${recipient.email}: ${errorMessage}`);
      results.push({
        email: recipient.email,
        success: false,
        provider: 'emailjs',
